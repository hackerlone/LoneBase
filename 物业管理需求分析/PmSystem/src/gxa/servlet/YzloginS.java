...
public class YzloginS extends HttpServlet {
	...
	public void checkname(PrintWriter out,HttpServletRequest request){
		...
		Yz yz = (Yz)conn.
		getOBJById("select * from [yz] where o_xm = '"+name+"'",
				new Yz());
		if(yz!=null){
		 out.println(1);
		}else{
	...
	public void login(PrintWriter out,HttpServletRequest request){
		...
		getOBJById("select * from [yz] where o_xm = '"+name+"'",
				new Yz());
		if(yz!=null){
			if(pwd.equals(yz.getO_pwd())){
				out.println(0);//登陆成功

			      HttpSession session=request.getSession();
			      HttpSession sessionName=request.getSession();
			     session.setAttribute("name","yz");
			     sessionName.setAttribute("yzname", name);
			}else{
				out.println(-2);//密码错误
			}
		}else{
